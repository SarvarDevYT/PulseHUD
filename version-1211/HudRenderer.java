...
public class HudRenderer {
    ...
    public static void renderHUD(DrawContext context, float tickDelta) {
        ...
        // Apply global scale (1.21.11+ uses Matrix3x2fStack with 2D scaling)
        context.getMatrices().pushMatrix();
        context.getMatrices().scale(config.hudScale, config.hudScale);
        ...
        if (config.showHotbar) {
            renderCustomHotbar(context, scaledCenterX, scaledBottomY, player, opacity, tickDelta);
        }
        if (config.showXp) {
            renderCustomXPBar(context, scaledCenterX, scaledBottomY, player, opacity);
        }
        if (config.showHealth || config.showArmor) {
            renderLeftPanel(context, scaledCenterX, scaledBottomY, player, opacity, config);
        }
        if (config.showHunger) {
            renderRightPanel(context, scaledCenterX, scaledBottomY, player, opacity, config);
        }
    ...
    private static void drawVignetteBorder(DrawContext context, int w, int h, int color) {
        context.fill(0, 0, w, 2, color);
        context.fill(0, h - 2, w, h, color);
        context.fill(0, 2, 2, h - 2, color);
        context.fill(w - 2, 2, w, h - 2, color);
    }

    private static void renderCustomHotbar(DrawContext context, int centerX, int bottomY, PlayerEntity player, float opacity, float tickDelta) {
        int x = centerX - 91;
        int y = bottomY - 23;
        int w = 182;
        int h = 22;
        ...
        context.fill((int) animX, y + 1, (int) animX + 19, y + 2, activeGlow);
        context.fill((int) animX, y + 20, (int) animX + 19, y + 21, activeGlow);
        context.fill((int) animX, y + 1, (int) animX + 1, y + 21, activeGlow);
        context.fill((int) animX + 19, y + 1, (int) animX + 20, y + 21, activeGlow);
    ...
    private static void renderCustomXPBar(DrawContext context, int centerX, int bottomY, PlayerEntity player, float opacity) {
        int x = centerX - 91;
        int y = bottomY - 28;
        int w = 182;
        int h = 4;
    ...
    private static void renderLeftPanel(DrawContext context, int centerX, int bottomY, PlayerEntity player, float opacity, PulseConfig config) {
        int x = centerX - 122;
        int y = bottomY - 60;
        int w = 110;
        int h = 30;
        ...
        if (config.showArmor) {
            ...
            int shieldX = finalX + 6;
            int shieldY = finalY + 4;
            int shieldColor = applyOpacity(0xFF00E5FF, opacity);
            
            int numShields = Math.min(10, (armorValue + 1) / 2);
            for (int i = 0; i < 10; i++) {
                if (i < numShields) {
                    context.fill(shieldX + i * 8, shieldY + 1, shieldX + i * 8 + 6, shieldY + 7, shieldColor);
                } else {
                    context.fill(shieldX + i * 8, shieldY + 3, shieldX + i * 8 + 6, shieldY + 5, applyOpacity(0x33FFFFFF, opacity));
                }
            }
        }
        ...
        if (config.showHealth) {
            ...
            if (barWidth > 0) {
                int hpColor = applyOpacity(0xFFFF2222, opacity);
                if (player.hasStatusEffect(StatusEffects.POISON)) {
                    hpColor = applyOpacity(0xFF7E8C24, opacity);
                } else if (player.hasStatusEffect(StatusEffects.WITHER)) {
                    hpColor = applyOpacity(0xFF262626, opacity);
                }
            ...
            if (absorption > 0) {
                ...
                context.fill(hpX, hpY + 4, hpX + absWidth, hpY + 6, applyOpacity(0xFFFFD700, opacity));
            }
    ...
    private static void renderRightPanel(DrawContext context, int centerX, int bottomY, PlayerEntity player, float opacity, PulseConfig config) {
        int x = centerX + 12;
        int y = bottomY - 60;
        int w = 110;
        int h = 30;
        ...
        if (foodWidth > 0) {
            int foodColor = applyOpacity(0xFFFF8A00, opacity);
            if (player.hasStatusEffect(StatusEffects.HUNGER)) {
                foodColor = applyOpacity(0xFF5A8442, opacity);
            }
